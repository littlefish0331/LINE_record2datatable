...
有一些 LINE 的 side project，需要先將對話紀錄表格化做儲存，才能後續分析。加上發現 LINE 現在備分聊天紀錄非常方便!!

所以決定針對這個首要步驟，建立一個專案執行維護並模組化，完善之後可以給其他 silde project 做加值使用。

鎖定要表格化的聊天視窗，可為「一對一、多人、有 chatbot 的群組、聊天群(非真的群組)」等等。  
...
- 電腦版 LINE，先拉到最上面(這一步一定要，不然只會存到暫時存的)，右鍵全選+複製，或是 Ctrl+A, Ctrl+C，再貼到 txt 記事本上。
- 電腦版 LINE，如果不往上拉，直接匯出只會匯出當前有 loading 的紀錄，如果越往上滑，則會有越多的紀錄可以匯出。所以需要全部的對話紀錄，  
- 手機上的LINE選擇備分聊天紀錄時，會直接匯出所有紀錄。右上角 hamburger menu > 其他設定 > 聊天設定 > 傳送聊天紀錄。

匯出結果來看，會有兩種。因此，本專案會考慮到不同來源，做不同的擷取模組。
...
範例資料明明就同一時段、同一聊天群組，但不知道為何資料大小有差異(127KB, 73KB)，應該是格式問題。
...
- <用戶>，會使用你個人修改的名稱，但在<訊息>中，雖然手機上看到你修改的用戶名稱，但實際資料轉出來時，會是用LINE上預設的名稱。
...
比如存檔名稱為「LINE-NCHC_Data_Analysis-YYYYMMDDTHHMM(from)-YYYYMMDDTHHMM(to)」
...
再從 raw.csv 的內文 context，抓取出新的特徵欄位。
